...
def SupConSimple(features, labels, temperature=0.07):
    """
    Compute contrastive loss for the features
    Args:
        features: tensor of shape [batch_size, feature_dim] or [batch_size, n_views, feature_dim]
    ...
    if len(features.shape) > 2:
        # Assume shape is [batch_size, n_views, feature_dim]
        batch_size = features.shape[0]
        n_views = features.shape[1]
        
        # Flatten views: [batch_size * n_views, feature_dim]
    ...
    mask_positive = mask_positive - torch.eye(mask_positive.shape[0], device=mask_positive.device) # remove diagonals 
